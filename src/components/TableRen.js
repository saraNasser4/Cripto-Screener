...
function TableRen () {
  ...
  return (
    ...
    {
      cryptoData ?
      <table className='border border-gray-100 rounded'>
          ...
          <tbody className='text-center text-base [&>*]:border-b [&>*]:border-gray-100'>
            ...
            {cryptoData.map(data => {
              ...
              const DataProps = {
                mobileScreen: [[data.image, data.symbol],	data.current_price],
                smScreen: [[data.image, data.symbol], data.name, data.current_price, data.market_cap_change_percentage_24h],
                mdScreen: [[data.image, data.symbol], data.name, data.current_price, data.total_volume,	data.market_cap_change_percentage_24h],
                lgScreen: [[data.image, data.symbol], data.name, data.current_price, data.total_volume,	data.market_cap_change_percentage_24h, data.price_change_percentage_1h_in_currency, data.price_change_percentage_24h_in_currency, data.price_change_percentage_7d_in_currency],
              }
              let dataToDisplay =[]
              for(let key in DataProps){
                if(DataProps[key].length === ScreenSize.length) dataToDisplay = DataProps[key]
              }
              

              return (
                <tr key={data.id} className='hover:bg-gray-200 [&>*]:p-4'>
                  <td className='flex items-center uppercase'>
                    <button className='outline-0 border-0'>
                    <svg className="w-[1.5rem] ml-1.5 hover:fill-cyan fill-gray-100" width="30" height="30" viewBox="0 0 30 30" fill="cyan" xmlns="http://www.w3.org/2000/svg">
                      ...
                      <defs>
                        <clip-path id="clip0_16_420">
                          <rect width="30" height="30" fill="white"/>
                        </clip-path>
                      </defs>
                    </svg> 
                    </button>
                    <img className="w-[1.2rem] h-[1.2rem] mx-1.5" src={dataToDisplay[0][0]} alt="icon logo" />
                    <span>{dataToDisplay[0][1]}</span>
                  </td>
                  {dataToDisplay[1] === data.name && <td>{dataToDisplay[1]}</td>} 
                  <td className='p-4'>{(data.current_price).toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2, style:"currency", currency:"USD"})}</td>
                  {dataToDisplay[3] === data.total_volume  && <td>{dataToDisplay[3]}</td>} 
                  {(dataToDisplay[3] === data.market_cap_change_percentage_24h || dataToDisplay[4] === data.market_cap_change_percentage_24h) && <td className={`${data.market_cap_change_percentage_24h >= 0 ? 'text-green' : 'text-red'}`}>%{data.market_cap_change_percentage_24h.toLocaleString('en-US', option)}</td>} 
                  {dataToDisplay[5] === data.price_change_percentage_1h_in_currency && <td className={`${data.price_change_percentage_1h_in_currency >= 0 ? 'text-green' : 'text-red'}`}>%{data.price_change_percentage_1h_in_currency.toLocaleString('en-US', option)}</td>} 
                  {dataToDisplay[6] === data.price_change_percentage_24h_in_currency && <td className={`${data.price_change_percentage_24h_in_currency >= 0 ? 'text-green' : 'text-red'}`}>%{data.price_change_percentage_24h_in_currency.toLocaleString('en-US', option)}</td>} 
                  {dataToDisplay[7] === data.price_change_percentage_7d_in_currency && <td className={`${data.price_change_percentage_7d_in_currency >= 0 ? 'text-green' : 'text-red'}`}>%{data.price_change_percentage_7d_in_currency.toLocaleString('en-US', option)}</td>} 
                </tr>
              )
            })}
          </tbody>
